...
define( 'SPP_PLUGIN_VERSION', 3.90 );
...
class SISANU_Popupate_Posts {
	...
	public static function get_plugin_settings() {
		...
		$images_initial_string  = plugins_url( '/assets/images/sample1.jpg', __FILE__ ) . chr( 13 ) . plugins_url( '/assets/images/sample2.jpg', __FILE__ ) . chr( 13 ) . plugins_url( '/assets/images/sample3.jpg', __FILE__ ) . chr( 13 ) . plugins_url( '/assets/images/sample4.jpg', __FILE__ ) . chr( 13 ) . plugins_url( '/assets/images/sample5.jpg', __FILE__ );
	 ...
	 * @return void
	 */
	public static function set_local_images_from_options( string $images_list ) {
	 ...
	 * @return string
	 */
	public static function trim_endings( $text ) {
	 ...
	 * @return int
	 */
	public static function get_random_string( $min, $max = 0, $lower = false ) {
	 ...
	 * @return int
	 */
	public static function get_random_number( $min = 0, $max = 0 ) {
	...
	public static function replace_text_tags( $text ) {
		...
		if ( substr_count( $text, '#[URL]' ) ) {
			$text = preg_replace_callback(
				'/\#\[URL\]/i',
				function ( $matches ) {
					$string = self::get_random_string( 2, 15 ) . '-' . str_replace( '', '-', self::get_random_string( 3, 15 ) ) . '.' . chr( 97 + mt_rand( 0, 25 ) ) . chr( 97 + mt_rand( 0, 25 ) );
		...
		if ( substr_count( $text, '#[s' ) ) {
			$text = preg_replace_callback(
				'/\#\[s\-([0-9]+)\:([0-9]+)\]/',
				function ( $matches ) {
					return self::get_random_string( $matches[1], $matches[2], true );
				},
				$text
			);
		}
		...
		if ( substr_count( $text, '#[S' ) ) {
			$text = preg_replace_callback(
				'/\#\[S\-([0-9]+)\:([0-9]+)\]/',
				function ( $matches ) {
					return self::get_random_string( $matches[1], $matches[2], false );
				},
				$text
			);
		}
		...
		if ( substr_count( $text, '#[N' ) ) {
			$text = preg_replace_callback(
				...
				function ( $matches ) {
					...
					$nr  = (string) self::get_random_number( $matches[1], $matches[2] );
					return str_pad( $nr, $max, $matches[3], STR_PAD_LEFT );
				},
				$text
			);
			...
			$text = preg_replace_callback(
				...
				function ( $matches ) {
					$max = strlen( $matches[2] );
					$nr  = (string) self::get_random_number( $matches[1], $matches[2] );
					return str_pad( $nr, $max, $matches[3], STR_PAD_RIGHT );
				},
				$text
			);
			...
			$text = preg_replace_callback(
				'/\#\[N\-([0-9]+)\:([0-9]+)\]/i',
				function ( $matches ) {
					return self::get_random_number( $matches[1], $matches[2] );
				},
				$text
			);
		}
		...
		if ( substr_count( $text, '#[L]' ) ) {
			$text = preg_replace_callback(
				'/\#\[L\]/',
				function ( $matches ) {
					return mb_strtoupper( chr( 97 + mt_rand( 0, 25 ) ) );
				},
				$text
			);
		}
	...
	public static function spp_images_mention() : string {
		return '<p><em>' . esc_html__( 'Images to be set randomly as featured image.', 'spp' ) . '</em></p>';
	}
	...
	public static function spp_show_plugin_images() {
		if ( ! empty( self::$settings['images_path'] ) ) {
			...
			if ( count( $p ) !== 0 ) :
				echo self::spp_images_mention(); // phpcs:ignore
				?>
				...
				id="spp_taxonomy<?php echo esc_attr( $mk ); ?>">
					<?php if ( ! empty( self::$allowed_taxonomies ) ) : ?>
						<?php foreach ( self::$allowed_taxonomies as $kk => $vv ) : ?>
							<option value="<?php echo esc_attr( $kk ); ?>"<?php selected( $kk, $val_tax ); ?>><?php echo esc_attr( $vv ); ?></option>
						<?php endforeach; ?>
					<?php endif; ?>
				</select>
	 ...
	 * @return boolean
	 */
	public static function has_pattern( $text ) {
			...
			<form id="spp_settings_frm" action="" method="post" class="spp">
				...
				<div class="options-boxes">
					...
					<div>
						...
						<textarea name="spp[images_list]" id="spp_images_list" cols="20" rows="4"></textarea>
						<div class="row-span four-one">
							<a onclick="sppCopyImagesList();"><span class="dashicons dashicons-arrow-up-alt"></span> <?php esc_html_e( 'use the plugin sample images', 'spp' ); ?></a>
						...
						<div id="spp_hint_patterns" class="spp_hint not-visible">
							...
							<p>
								...
								echo sprintf(
									// Translators: %1$s - letter pattern, %2$s - number pattern, %3$s - text pattern.
									__( 'Pattern examples: %1$s, %2$s, %3$s, %4$s, %5$s, %6$s, %7$s, %8$s, %9$s, %10$s.', 'spp' ),
									'<a onclick="sppApplyPattern(\'#[MOBILE]\');">' . __( 'mobile number', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[EMAIL]\');">' . __( 'email address', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[URL]\');">' . __( 'random URL', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[l]\');">' . __( 'letter', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[L]\');">' . __( 'capital letter', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[N-0:100]\');">' . __( 'number between 0-100', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[N-0:100:L0]\');">' . __( 'with leading 0', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[N-0:100:T0]\');">' . __( 'with trailing 0', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[L]#[s-1:16] #[L]#[s-1:16]\');">' . __( 'random title', 'spp' ) . '</a>',
									'<a onclick="sppApplyPattern(\'#[S-35:220]. #[S-35:220]. #[S-35:220].\');">' . __( 'random text', 'spp' ) . '</a>'
								);
								?>
							</p>
							<p>
	...
	public static function execute_add_random_posts() {
		...
		for ( $i = 0; $i < self::$settings['max_number']; $i ++ ) {
			...
			if ( ! empty( $name ) ) {
				...
				$name = self::replace_text_tags( $name );
